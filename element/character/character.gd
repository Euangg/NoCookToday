...
#控制信号
var control_float_x:float=0
var control_pressed_jump:bool=false
var control_pressed_attack:bool=false
var control_pressed_down:bool=false
var control_released_down:bool=false
...
@onready var timer_down: Timer = $Timer_Down
...
func _physics_process(delta: float) -> void:
	...
	match state_:
		State.IDLE:
			if is_zero_approx(control_float_x):pass
			else:nextState=State.RUN
			if velocity.y>0:nextState=State.FALL
			if velocity.y<0:nextState=State.RISE
			if control_pressed_attack:nextState=State.ATTACK
		State.RUN:
			if is_zero_approx(control_float_x):nextState=State.IDLE
			if velocity.y>0:nextState=State.FALL
			if velocity.y<0:nextState=State.RISE
			if control_pressed_attack:nextState=State.ATTACK
		State.RISE:
	...
	if state_==State.HURT:
		...
		match state_:
			State.IDLE:
				velocity.x=0
				if control_pressed_jump:Jump()
			State.RUN:
				velocity.x=control_float_x*speedRun*buff
				if control_pressed_jump:Jump()
			State.RISE:
				velocity.x=control_float_x*speedRun*buff
				if control_pressed_jump:
					if skyJumpNum_>0:
						skyJumpNum_=0
						var sfx=Global.SFX_AWAKE.instantiate()
						add_child(sfx)
						velocity.y=-400
						showGhost=true
			State.FALL:
				...
				if control_pressed_jump:
					if skyJumpNum_>0:
						...
						showGhost=true
			State.ATTACK:
				if isOnFloor&&control_pressed_jump:Jump()
				velocity.x=control_float_x*speedRun*buff
		if not is_zero_approx(control_float_x):
			direction_=Direction.LEFT if control_float_x<0 else Direction.RIGHT
		
	if control_pressed_down:
		timer_down.start()
		set_collision_mask_value(4,0)
	if control_released_down:
		timer_down.stop()
		timer_down.timeout.emit()
	state_time_+=delta
	velocity.y+=1000*delta
	move_and_slide()

func Jump()->void:
	var sfx=Global.SFX_JUMP.instantiate()
	add_child(sfx)
	velocity.y=-500
